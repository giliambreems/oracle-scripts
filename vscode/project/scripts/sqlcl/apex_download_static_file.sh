...
#
# - For a static workspace/application file to download, run as:
#     './scripts/sqlcl/apex_download_static_file.sh "<FILE>"'
#
...
echo Run SQLcl to download static file: ${STATIC_FILE_NAME}
echo Connect to database using Oracle Wallet \"${CONNECTION_STRING}\"
sql -l ${CONNECTION_STRING} @scripts/sqlcl/apex_download_static_file.sql "${STATIC_FILE_NAME}" "${APEXWORKSPACE}"
